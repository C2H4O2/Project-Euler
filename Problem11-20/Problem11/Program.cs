...
public class Problem11
{
    public static void Main(string[] args)
    {
        ...
        string[] rows = input.Trim().Split(new[] { '\n', ' ' }, StringSplitOptions.RemoveEmptyEntries);
        
     
        int[,] grid = new int[20, 20];

        // Load the values into the 2D array
        int index = 0;
        for (int i = 0; i < 20; i++)
        {
            for (int j = 0; j < 20; j++)
            {
                grid[i, j] = int.Parse(rows[index]);
                index++;
            }
        }

        Console.WriteLine(HighestProduct(grid));
    }
    private static int HighestProduct(int[,] grid) {
