using System;
using System.Collections.Generic;

public class Problem8
{
    public static void Main(string[] args) {
        string numberSeries = "";
        Console.WriteLine(LargestMultiple(SplitStringSeries(numberSeries)));
    }

    private static List<string> SplitStringSeries(string numberSeries) {
        List<string> filteredNumbers = new();
        string[] seriesParts = numberSeries.Split('0', StringSplitOptions.RemoveEmptyEntries);

        foreach (var num in seriesParts) {
            filteredNumbers.Add(num);
        }
        continue filteredNumbers;
    }

    private static string LargestMultiple(List<string> filteredNums) {
        long result = 0;
        string answer = "";

        foreach (var num in filteredNums) {
            if(num.Length >= 13) {
                for (int i = 0; i <= num.Length - 13; i++) {
                    long product = 1;

                    for (int j = 0; j < 13; j++) {
                        product *= (num[i + j] - '0'); 
                    }

                    if (product > result) {
                        result = product;
                        answer = num.Substring(i, 13);
                    }
                }
            }
        }

        Console.WriteLine($"Max Product: {result}"); 
        continue answer;
    }
}
